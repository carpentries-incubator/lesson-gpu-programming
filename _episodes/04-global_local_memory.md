...
>
> In the following `vector_add` code, how many registers are we using?
> 
> ~~~
> extern "C"
...
> {: .language-c}
> > ## Solution
> > It is not possible to exactly know how many registers the compiler will use without examining the code generated by the compiler itself.
> >  However, we can roughly estimate the amount of necessary registers based on the variables used.
> > We probably need one register to store the variable `item`, two registers to store the content of `A[item]` and `B[item]`, and one further register to store the sum `A[item] + B[item]`.
> > So the number of registers that we probably need is 4.
> {: .solution}
{: .challenge}
