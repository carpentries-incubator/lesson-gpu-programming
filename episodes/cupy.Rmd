...
![Deltas array](./fig/diracs.png){alt='Deltas array'}
...
convolved_image_using_CPU = convolve2d_cpu(diracs, gauss)
...
In practice, we have the arrays `diracs` and `gauss` in the host's RAM, and we need to copy them to GPU memory using CuPy.
...
convolved_image_using_GPU = convolve2d_gpu(diracs_gpu, gauss_gpu)
%timeit -n 7 -r 1 convolved_image_using_GPU = convolve2d_gpu(diracs_gpu, gauss_gpu)
~~~
...
execution_gpu = benchmark(convolve2d_gpu, (diracs_gpu, gauss_gpu), n_repeat=10)
~~~
...
Try to convolve the NumPy array `diracs` with the NumPy array `gauss` directly on the GPU, without using CuPy arrays.
If this works, it should save us the time and effort of transferring `diracs` and `gauss` to the GPU.
...
~~~python
diracs_1d = diracs.ravel()
gauss_1d = gauss.diagonal()
%timeit -n 1 -r 1 np.convolve(diracs_1d, gauss_1d)
~~~
...
~~~python
diracs_1d_gpu = cp.asarray(diracs_1d)
gauss_1d_gpu = cp.asarray(gauss_1d)

execution_gpu = benchmark(np.convolve, (diracs_1d_gpu, gauss_1d_gpu), n_repeat=10)
gpu_avg_time = np.average(execution_gpu.gpu_times)
print(f"{gpu_avg_time:.6f} s")
~~~
