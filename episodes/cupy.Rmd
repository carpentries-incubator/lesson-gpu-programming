...
These measurements are also more stable and representative, because `benchmarks` disregards the compile time and the repetitions warm up the GPU.
...
Try to convolve the NumPy array `deltas` with the NumPy array `gauss` directly on the GPU, that is, without CuPy arrays.
...
To check that the host and the device actually produced the same output, we compare the two output arrays `convolved_image_using_GPU` and `convolved_image_using_CPU` as follows:
...
Left and right in the data will be swapped with the method `fliplr` just to adhere to the astronomical convention of the [right ascension](https://en.wikipedia.org/wiki/Right_ascension) increasing leftwards.
